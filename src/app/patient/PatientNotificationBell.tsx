...
import { markAllPatientNotificationsReadAction, markPatientNotificationReadAction } from "./notification-actions";

export interface PatientNotificationRow {
  id: string;
  type: string;
  message: string;
  link: string | null;
...
// Same bell/dropdown pattern as dashboard/NotificationBell.tsx, adapted to
// manage its own open state (the patient nav has no shared NavMenus
// orchestrator) and to mark-read against the patient notification actions.
export default function PatientNotificationBell({ notifications }: { notifications: PatientNotificationRow[] }) {
  const [open, setOpen] = useState(false);
  const [locallyRead, setLocallyRead] = useState<Set<string>>(new Set());
  const [, startTransition] = useTransition();

  const isRead = (n: PatientNotificationRow) => n.read || locallyRead.has(n.id);
  ...
  function markOneRead(id: string) {
    ...
    startTransition(() => {
      ...
      fd.append("id", id);
      markPatientNotificationReadAction(fd);
    });
  }
