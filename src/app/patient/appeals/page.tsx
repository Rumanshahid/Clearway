...
export default async function PatientAppealsPage({
  ...
  const { data: requests } = await supabase
    ...
    .select("id, claim_number, denial_reason, letter_content, status, created_at, doctor_profile_id")
    ...
    <div className="max-w-[720px] mx-auto px-5 sm:px-10 py-10">
      ...
      {doctors.length === 0 ? (
        <div className="card p-6 text-[13.5px] text-gray-500">
          ...
          <a href="/patient/profile" className="text-indigo-600 font-medium">Profile</a> before requesting an appeal.
        </div>
      ) : (
          ...
          <details key={r.id} className="card p-4">
            ...
            {r.letter_content ? (
              <div className="mt-4 pt-4" style={{ borderTop: "1px solid var(--gray-200)" }}>
                <p className="text-[11px] uppercase tracking-wide text-gray-400 font-semibold mb-2">Your draft appeal letter — for your doctor&apos;s office to review</p>
