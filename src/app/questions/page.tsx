...
export default async function QuestionsListPage({
  ...
  return (
    <div className="landing-root">
      <SiteNav />
      <div className="max-w-[1440px] mx-auto px-5 sm:px-10 py-14">
        ...
        <div className="flex gap-6 items-start mt-8">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-6">
            {(questions || []).map((q) => (
              <Link key={q.id} href={`/questions/${q.id}`} className="card p-5 flex flex-col gap-2 hover:bg-gray-50 transition-colors">
          ...
          <RecentQuestionsSidebar questions={recent || []} />
        </div>
      </div>
      <SiteFooter />
      <LandingScripts />
