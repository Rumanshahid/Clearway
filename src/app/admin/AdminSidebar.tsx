...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-[68px] flex-shrink-0 flex flex-col items-center py-4 gap-1" style={{ background: "var(--navy-900)" }}>
      ...
      <Link href="/doctor/dashboard" title="Back to app" aria-label="Back to app" className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ color: "#A0A8C0" }}>
        <ArrowLeftIcon />
      </Link>
      <form action={signOutAction}>
        <button type="submit" title="Sign out" aria-label="Sign out" className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ color: "#A0A8C0" }}>
          <LogOutIcon />
        </button>
      </form>
    </aside>
