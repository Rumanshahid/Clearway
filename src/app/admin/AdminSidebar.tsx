...
// and the first nav icon -- the toggle sits in an outer wrapper alongside
// (not inside) <aside>, see DashboardSidebar.tsx for why.
export default function AdminSidebar() {
  const pathname = usePathname();
  const [expanded, setExpanded] = useState(false);

  const linkClass = (isExpanded: boolean) =>
    `rounded-xl flex items-center transition-colors flex-shrink-0 ${
      isExpanded ? "w-full h-11 justify-start gap-3 px-3 hover:bg-white/10" : "w-11 h-11 justify-center transition-transform hover:scale-110 active:scale-95"
    }`;

  return (
    <div className={`relative flex-shrink-0 transition-[width] duration-200 ${expanded ? "w-[200px]" : "w-[68px]"}`}>
      <aside
        className={`w-full flex flex-col items-center py-4 gap-1 sticky top-0 h-screen overflow-y-auto ${expanded ? "items-stretch px-3" : ""}`}
      ...
      >
        ...
        <Link href="/doctor/dashboard" title="Back to app" aria-label="Back to app" className={linkClass(expanded)} style={{ color: "#A0A8C0" }}>
        ...
        <form action={signOutAction} className={expanded ? "w-full" : undefined}>
          <button type="submit" title="Sign out" aria-label="Sign out" className={linkClass(expanded)} style={{ color: "#A0A8C0" }}>
      ...
      <button
        ...
        className="absolute top-[52px] w-6 h-6 rounded-full flex items-center justify-center transition-transform hover:scale-110 z-10"
