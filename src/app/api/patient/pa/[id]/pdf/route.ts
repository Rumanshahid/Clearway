...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: pa } = await admin
    ...
    .maybeSingle();
  if (!pa?.letter_content) return NextResponse.json({ error: "Letter not found" }, { status: 404 });
  ...
  const buffer = await renderToBuffer(
    PatientLetterPdfDocument({
      title: "Prior Authorization Request Letter",
      patientReference: account?.patient_ref_id || "",
      content: pa.letter_content,
    })
  );
  ...
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      ...
      "Content-Disposition": `inline; filename="asaanbil-pa-${account?.patient_ref_id || id}.pdf"`,
    },
  });
}
